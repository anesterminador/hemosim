      ...
      groups: [
        ...
        { name: 'Hepáticas', items: [
          ...
          { key:'shp', lbl:'Síndrome hepatopulmonar (SHP)',
            ...
            notes:   'Defeito de oxigenação puro (PaO₂ <80, A-a >15) por shunt intrapulmonar. NÃO eleva PAP — diferente de POPH. 5-32% dos cirróticos avançados. Oxigenação ainda não é dimensão modelada.',
            refs:    ['NBK562169'],
          },
      ...
      groups: [
        ...
        { name: 'Obstrutivas', items: [
          ...
          { key:'asma_grave', lbl:'Asma grave / mal controlada',
            effects: {pvr:+0.15, chronotropy:+0.10},
          },
          { key:'dpoc_12', lbl:'DPOC GOLD 1–2 (leve a moderado)',
            effects: {pvr:+0.10},
          },
          { key:'dpoc_34', lbl:'DPOC GOLD 3–4 (grave a muito grave)',
            effects: {pvr:+0.55, chronotropy:+0.15, preload:+0.20},
            waves:   {pap:{p:'hap_primaria', prio:30}},
            notes:   'PAP ~25-35 leve; sinais incipientes de sobrecarga VD.',
          },
          { key:'dpoc_corpulm', lbl:'DPOC com cor pulmonale',
            effects: {pvr:+1.50, preload:+0.80, rv_contractility:-0.35, contractility:-0.05, chronotropy:+0.25},
            waves:   {pap:{p:'hap_primaria', prio:50}},
            notes:   'VD sobrecarregado e disfuncional; PAP ~30-50, PVC ~15-22, RVEF ↓↓ (~25-35%); HAP secundária à hipoxemia crônica. VE preservado.',
          },
        ...
        { name: 'Restritivas', items: [
          { key:'fpi', lbl:'Fibrose pulmonar idiopática',
            effects: {pvr:+0.50, chronotropy:+0.20, preload:+0.20},
            waves:   {pap:{p:'hap_primaria', prio:25}},
            notes:   'Restrição grave; PAP ~20-32, taqui em repouso por hipoxemia.',
          },
          { key:'dpi', lbl:'Doença pulmonar intersticial',
            effects: {pvr:+0.35, chronotropy:+0.10},
          ...
          { key:'soh', lbl:'Síndrome obesidade-hipoventilação',
            effects: {pvr:+0.55, preload:+0.30, chronotropy:+0.15, afterload:+0.20},
            waves:   {pap:{p:'hap_primaria', prio:25}},
            notes:   'Hipóxia + hipercapnia crônicas; HAP secundária + HAS associada; PAP ~22-35.',
          },
        ]},
        ...
        { name: 'Agudas / sequelas', items: [
          { key:'pneumonia', lbl:'Pneumonia',
            effects: {chronotropy:+0.25, pvr:+0.15, afterload:-0.10, contractility:+0.05, permeability:+0.30},
            notes:   'Estado pré-séptico em casos graves: hiperdinâmico, taqui, vasodilatação leve, EVLW levemente ↑.',
          },
          { key:'sdra', lbl:'SDRA / SARA',
            effects: {pvr:+0.80, chronotropy:+0.30, contractility:-0.05, permeability:+2.5},
            waves:   {pap:{p:'hap_primaria', prio:35}},
            notes:   'Edema de permeabilidade (não cardiogênico): EVLW 15-30, PVPI >3 (defining feature). PAP elevada (vasoconstrição hipóxica + edema); taqui marcada.',
          },
          { key:'covid_seq', lbl:'COVID-19 — sequela pulmonar',
            effects: {pvr:+0.10},
          },
          { key:'pneumotorax', lbl:'Pneumotórax recente',
            effects: {preload:-0.10},
          },
